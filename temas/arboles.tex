...
Para cada arista seleccionada, se fija si la misma forma un ciclo con las que ya seleccion\'o. Si no lo forma, la inserta en el AGM. El algoritmo termina cuando seleccionaron $n-1$ aristas. Si el grafo no es conexo, forma un \emph{``bosque generador m\'inimo''}. La complejidad del mismo es O(n log m), pero hay que cumplir un par de requisitos t\'ecnicos para alcanzarla:
